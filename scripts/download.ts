/**
 * The Download class is responsible for downloading Salesforce CLI command documentation in JSON format.
 * It fetches the metadata, processes the content, and stores it in a specified directory for further use.
 */
...
class Download {
  ...
  public async run() {
    ...
    const response = await this.getMetaFile();
    await this.getContent(response.toc[0].children, response.deliverable, response.version.doc_version);
    ...
    for (const file of files) {
      const topics = await this.process(`${RAW_CONTENT_PATH}/${file}`);
  ...
  public async process(path = "data/raw/content/cli_reference_alias_set.htm.json") {
    // read the json file
